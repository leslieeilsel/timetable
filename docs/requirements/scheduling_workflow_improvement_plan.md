...
下表中路径均相对于 `/semesters/:semesterId`；新增目标是待实施方案。
...
| 批次 | 优先级 | 工作内容 | 依赖与范围 |
| --- | --- | --- | --- |
| A：入口和职责 | P0 | 移除误导性步骤条；准备卡片对应真实页面；提供同名菜单；修正按钮文案；固定安排独立入口；兼容旧路由 | 以页面、路由和组件复用为主，不改求解器 |
| B：连续编排 | P1 | 以 `/planning` 为工作台，整合生成入口、候选采用、待排课程和问题定位；保留草稿、视角和返回状态 | 基于 A；复用已有编辑、生成及诊断能力 |
| C：状态与发布统一 | P1 | 草稿/正式/历史语义统一；统一发布核对；明确生效范围；核实候选和编辑草稿入口行为一致 | 与 B 配合；核对发布契约及已有调课迁移逻辑 |
| D：规则和落位反馈 | P2 | 常用时间规则网格；选课后显示可放位置；具体冲突原因与规则定位 | 先盘点可用校验接口，缺少结构化原因时补充契约与 API |
| E：日常操作衔接 | P2 | 从实际课程直接发起调课；带入上下文；统一调整结果返回与查看 | 复用现有单次、持续调课和请假代课流程 |
...
以下是未来实施完成后的验收要求，不是本次已执行的检查结果。本次仅提交已有代码并编写文档，按用户要求未运行测试。

- [ ] 准备卡片、菜单和面包屑同名同目标；每张功能卡片打开可实际维护的页面。
